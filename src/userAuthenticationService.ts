 ...
 */
export function setSavingsCardEnrolledYear(year: number | null): void {
  savingsCardEnrolledYear = year;
}

/**
 * Gets the savings card enrolled year.
 ...
 * Use this in functions that require enrollment year for calculations.
 */
export function requireSavingsCardEnrolledYear(): number {
  if (!savingsCardEnrolledYear) {
    throw new Error('Savings card enrollment year not available.');
  }
  return savingsCardEnrolledYear;
}
